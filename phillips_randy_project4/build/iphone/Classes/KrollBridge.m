...
NSString * phillips_randy_project4$ModuleRequireFormat = @"(function(exports){"
...
@implementation phillips_randy_project4Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_phillips_randy_project4);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create phillips_randy_project4 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "phillips_randy_project4" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_phillips_randy_project4 = [[phillips_randy_project4Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_phillips_randy_project4];
	
	NSString *_phillips_randy_project4NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _phillips_randy_project4NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_phillips_randy_project4 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:phillips_randy_project4$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.phillips_randy_project4.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.phillips_randy_project4.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
