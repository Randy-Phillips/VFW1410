...
NSString * phillips_randy_project33$ModuleRequireFormat = @"(function(exports){"
...
@implementation phillips_randy_project33Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_phillips_randy_project33);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create phillips_randy_project33 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "phillips_randy_project33" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_phillips_randy_project33 = [[phillips_randy_project33Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_phillips_randy_project33];
	
	NSString *_phillips_randy_project33NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _phillips_randy_project33NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_phillips_randy_project33 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:phillips_randy_project33$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.phillips_randy_project33.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.phillips_randy_project33.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
