...
NSString * rPhillips_Project11$ModuleRequireFormat = @"(function(exports){"
...
@implementation rPhillips_Project11Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_rphillips_project11);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create rPhillips_Project11 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "rPhillips_Project11" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_rphillips_project11 = [[rPhillips_Project11Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_rphillips_project11];
	
	NSString *_rphillips_project11NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _rphillips_project11NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_rphillips_project11 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:rPhillips_Project11$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.rphillips_project11.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.rphillips_project11.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
