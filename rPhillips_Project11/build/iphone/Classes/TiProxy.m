...
//Rare exceptions to indicate a bug in the _rphillips_project11 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_rphillips_project11 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_rphillips_project11]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.rphillips_project11.module"
