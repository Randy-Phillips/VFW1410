...
//Rare exceptions to indicate a bug in the _phillips_randy_project1 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_phillips_randy_project1 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_phillips_randy_project1]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.phillips_randy_project1.module"
