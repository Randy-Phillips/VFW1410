...
NSString * phillips_randy_project1$ModuleRequireFormat = @"(function(exports){"
...
@implementation phillips_randy_project1Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_phillips_randy_project1);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create phillips_randy_project1 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "phillips_randy_project1" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_phillips_randy_project1 = [[phillips_randy_project1Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_phillips_randy_project1];
	
	NSString *_phillips_randy_project1NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _phillips_randy_project1NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_phillips_randy_project1 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:phillips_randy_project1$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.phillips_randy_project1.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.phillips_randy_project1.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
