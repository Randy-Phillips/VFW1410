...
NSString * phillips_randy_project2$ModuleRequireFormat = @"(function(exports){"
...
@implementation phillips_randy_project2Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_phillips_randy_project2);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create phillips_randy_project2 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "phillips_randy_project2" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_phillips_randy_project2 = [[phillips_randy_project2Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_phillips_randy_project2];
	
	NSString *_phillips_randy_project2NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _phillips_randy_project2NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_phillips_randy_project2 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:phillips_randy_project2$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.phillips_randy_project2.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.phillips_randy_project2.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
